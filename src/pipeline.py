...
def run(
    ...
    # Step 2 — structured financial extraction.
    periods = XBRLParser(filing).extract_periods()
    ...
    ctx = build_context(filing, current, prior, sections)
    ...
    if write:
        result["artifacts"] = _persist(
            output_root or _DEFAULT_OUTPUT, filing, periods, sections, memo
        )
    return result
...
def _persist(
    ...
    safe_fy = (memo.fiscal_year or "unknown").replace("/", "-")
    out_dir = Path(output_root) / filing.ticker / safe_fy
